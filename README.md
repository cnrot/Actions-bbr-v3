...
脚本会优先安装并运行 Ookla 官方 `speedtest`，要求用户在中国移动、中国联通、中国电信中选择一个运营商，然后通过 Speedtest server API 查找中国境内匹配测速节点并获取上传带宽、下载带宽和 Ping；如果找不到匹配节点，会降级为香港节点，最后才使用 Ookla 自动选择。测速失败时会提示手动输入上传带宽。

优化逻辑：

- 自动启用 `bbr` 拥塞控制和 `fq` 队列算法。
- 测速节点优先选择中国境内节点，并要求用户选择三大运营商之一。
