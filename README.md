...
一个同时支持 Debian/Ubuntu 和 RHEL 系（Rocky / Alma / CentOS / RHEL）VPS 的 BBRv3 内核安装与网络加速管理脚本。
...
脚本会自动识别当前系统架构和发行版类型，从本仓库 GitHub Releases 下载匹配的内核包（`.deb` 或 `.rpm`），并提供安装、指定版本安装、状态检查、加速模式切换和卸载功能。安装内核时可选择标准 BBRv3 或 BBRv3 Max 激进吞吐内核。

## 支持环境

| 项目 | 要求 |
| --- | --- |
| Debian 系 | Ubuntu 24.04+ / Debian 12+ |
| RHEL 系 | Rocky Linux 9+ / AlmaLinux 9+ / CentOS Stream 9+ / RHEL 9+ |
| 包管理器 | `apt-get`（Debian 系） / `dnf`（RHEL 系） |
| 架构 | `x86_64`（RHEL 系） / `x86_64` + `aarch64`（Debian 系） |
| 引导方式 | 建议使用 GRUB / GRUB2 |
| 使用场景 | VPS / 云服务器 / 独立服务器 |

不建议在树莓派、NanoPi 等依赖 U-Boot 或厂商定制内核链路的设备上使用。此类设备的内核安装和启动流程通常与通用 Linux VPS 不一致。
...
curl -fsSL -o cve_2026_31431_detector.py https://raw.githubusercontent.com/cnrot/Actions-bbr-v3/main/cve_2026_31431_detector.py
...
`.deb`（Debian 系）和 `.rpm`（RHEL 系）内核包由 GitHub Actions 构建并发布到本仓库 Releases。
...
- 强制默认启用 BBR 和 fq。
- 关闭 debug info。
- Debian 系产出 `.deb`，x86_64 额外产出 `.rpm`。
...
<a href="https://star-history.com/#cnrot/Actions-bbr-v3&Timeline">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=cnrot/Actions-bbr-v3&type=Timeline&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=cnrot/Actions-bbr-v3&type=Timeline" />
   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=cnrot/Actions-bbr-v3&type=Timeline" />
 </picture>
</a>
